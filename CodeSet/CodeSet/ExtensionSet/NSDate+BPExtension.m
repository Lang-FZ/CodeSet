...
/** 不加时区 */
- (void)bp_noZone_HourSystem_12_24 {
    [self.bp_formatter setLocale:[NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"]];
}
/** 不加时区 */
- (void)bp_noZone_Chinese {
    [self.bp_formatter setLocale:[NSLocale localeWithLocaleIdentifier:@"zh-Hans"]];
}
...
- (NSString *)bp_noZone_Month_M:(bp_Month_Type)type {
    if (type == bp_Month_M) {
        ...
        [self.bp_formatter setDateFormat:@"MMM"];
        [self bp_noZone_Chinese];
    } else if (type == bp_Month_MMM_en) {
        [self.bp_formatter setDateFormat:@"MMM"];
        [self bp_noZone_HourSystem_12_24];
    } else if (type == bp_Month_MMMM_zh) {
        [self.bp_formatter setDateFormat:@"MMMM"];
        [self bp_noZone_Chinese];
    } else if (type == bp_Month_MMMM_en) {
...
- (NSString *)bp_YMDHM {
    ...
    [self.bp_formatter setDateFormat:@"yyyy-MM-dd HH:mm"];
    self.bp_formatter.lenient = YES;
    [self.bp_formatter setTimeZone:[[NSTimeZone alloc] initWithName:@"Asia/Shanghai"]];
    [self.bp_formatter setCalendar:[[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian]];
    
    NSString *date_str = [self.bp_formatter stringFromDate:self];
    
    if (![date_str isEqualToString:@""] && (date_str != nil)) {
        return [NSString stringWithFormat:@"%@ (GMT+08:00)",date_str];
    } else {
        return @"";
    }
}
...
- (NSString *)bp_Format:(NSString *)format {
    ...
    [self.bp_formatter setDateFormat:format];
    [self.bp_formatter setTimeZone:[[NSTimeZone alloc] initWithName:@"Asia/Shanghai"]];
